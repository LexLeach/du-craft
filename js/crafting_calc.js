...
function itemRecipe(name, data) {
  ...
  this.getIngredients = function () {
    ...
    Object.keys(this.input).forEach(function (name, i, a) {
      if (!name.includes("Schematic Copy")) {
        // Skip ingredients with "Schematic Copy"
...
function isSkillApplicable(skill, recipe) {
  ...
  switch (skill.subject) {
    case "Tier":
      return (
        recipe.tier == skill.tier &&
        recipe.type == skill.type &&
...
function recipeCalc(recipes) {
  ...
  this.resetItemStats = function () {
    Object.keys(this.db).forEach(function (name, i) {
      this.db[name].actualOQ = JSON.parse(
        JSON.stringify(this.db[name].outputQuantity)
      );
      this.db[name].actualInput = JSON.parse(
        JSON.stringify(this.db[name].input)
      );
      this.db[name].actualTime = JSON.parse(JSON.stringify(this.db[name].time));
      this.db[name].actualB = JSON.parse(
        JSON.stringify(this.db[name].byproducts)
      );
    }, this);
  };
  ...
  this.simulate = function (input, inventory) {
    ...
    for (var jj = 0; jj < input.length; jj++) {
      ...
      if (debuggerEnabled === true) {
        this.debug.push(
          "number of " + iqPair.name + " required " + iqPair.quantity
        );
        this.debug.push(
          "checking inventory " + JSON.stringify(inventory[iqPair.name])
        );
      }
      if (
        iqPair.quantity <=
        inventory[iqPair.name].quantity + inventory[iqPair.name].bpquantity
      ) {
        if (debuggerEnabled === true) {
          this.debug.push(
            "inventory has enough of input, moving on to next input"
          );
        }
        continue;
      }
      ...
      if (ingredients.length != 0) {
        if (debuggerEnabled === true) {
          this.debug.push(
            iqPair.name +
              ": " +
              inventory[iqPair.name].quantity +
              " of " +
              iqPair.quantity
          );
        }
        while (
          ...
          byproducts.forEach(function (bPair, i) {
            ...
            itemSequence = itemSequence.concat([
              { name: bPair.name, bpquantity: bPair.quantity },
            ]);
          }, this);
        }
      } else {
        if (debuggerEnabled === true) {
          this.debug.push(
            "this is a base recipe, inserting desired amount to inventory"
          );
        }
        ...
        if (debuggerEnabled === true) {
          this.debug.push(
            "have " + inventory[iqPair.name].quantity + " of " + iqPair.quantity
          );
        }
  ...
  this.calcList = function (input, inv) {
    ...
    var compressedList = this.reduceItems(
      JSON.parse(JSON.stringify(craftList))
    );
    ...
    function populate(k, i) {
      ...
      if (
        this.db[k.name].type === "Pure" ||
        this.db[k.name].type === "Product" ||
        this.db[k.name].type === "Fuel"
      ) {
