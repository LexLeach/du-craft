...
const version = "1";
const lastUpdateTime = "10/5/2024";
...
loadJSON("/du-craft/data/itemsAccordion.json", function (json) {
  itemsAccordion = JSON.parse(json);
});
loadJSON("/du-craft/data/recipes.json", function (json) {
  recipes = JSON.parse(json);
});
loadJSON("/du-craft/data/skillsAccordion.json", function (json) {
  skillTree = parseSkillFile(JSON.parse(json));
});
loadJSON("/du-craft/data/orePrices.json", function (json) {
...
function calculate() {
  ...
  while (queueListDetailed.children.length > queueListDetailedCols) {
    queueListDetailed.removeChild(
      queueListDetailed.children[queueListDetailedCols]
    );
  }
  if (craft.length === 0) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    const quantityFractionDigits = [
      "Ore",
      "Pure",
      "Product",
      "Catalyst",
    ].includes(list[i].type)
      ? 2
      : 0;
    if (list[i].type === "Ore" || list[i].type === "Relic") {
      ...
      //   price.innerHTML = formatNum(
      //     Math.ceil(list[i].quantity / list[i].actualOQ) *
      //       schematicsPrices[list[i].name] *
      //       (1 - costReduction),
      //     0
      //   );
      //   orePrice +=
...
function addHoverSimple(item, digits) {
  ...
  output.innerHTML +=
    "batch size: " + formatNum(item.actualOQ, digits) + "<br/>";
...
function addHover(item, digits) {
  ...
  output.innerHTML +=
    "batch size: " + formatNum(item.actualOQ, digits) + "<br/>";
  output.innerHTML +=
    "batch count: " + Math.ceil(item.quantity / item.actualOQ);
  ...
  for (var i = 0; i < list.length; i++) {
    if (typeof list[i] == "object") {
      ...
      if (
        filter === "" ||
        cc
          .trans(language, list[i])
          .toLowerCase()
          .search(filter.toLowerCase()) !== -1 ||
        override
      ) {
        output.push(
          tab + "\t<div class='accordion-item unselectable " + cn + "'>"
        );
...
function updateSkillTreeDiv() {
  ...
  if (skillValues.onlyApplicableSkills) {
    skillTree.forEach((category) => {
      ...
      category.groups.forEach((group) => {
        ...
        group.skills.forEach((skill) => {
          if (shouldDisplaySkill(skill)) {
            ...
            const skillElement = skillTreeDiv.querySelector(
              '[skill-id="' + skill.id + '"]'
            );
            if (skillElement) {
        ...
        if (emptyGroup) {
          const groupElement = skillTreeDiv.querySelector(
            '[skill-group-id="' + group.id + '"]'
          );
          if (groupElement) {
      ...
      if (emptyCategory) {
        const categoryElement = skillTreeDiv.querySelector(
...
function updateSkills() {
  ...
  for (let i = 0; i < skillInputs.length; i++) {
    ...
    skillInput.checked =
      parseInt(skillInput.value) === skillValues.getValue(skillId);
  }
  cc.updateSkills(getAllSkills(), skillValues);
  calculate();
}
...
function tryRestoreState(profile) {
  ...
  try {
    ...
    if (state.version != version) {
      alert(
        "Old profile detected. You may have to reset the calculator (button at the top) to get it to work."
      );
    }
